...
CLI, HTTP API, and language SDKs are separate repositories that wrap this engine, this repo is the core library only. SMS OTP, SAML, organizations/multi-tenancy, SSO via OIDC, RBAC/permissions, and data export/delete-my-data are all explicitly out of scope for the current backlog pending an explicit go-ahead from the project owner. Passkeys are currently second-factor only, passwordless *primary* login via passkeys (no password step at all) is a planned fast-follow now that magic-link forced the shared "login without a password" plumbing to exist.
